...
async function loopTest(allBatches) {
    for(const batchD of allBatches){
        batchData = batchD;
        ...
        for(const ids of combination){
            await test(formatDate(yesterday), formatDate(today), ids);
        }
        try{
            await axios.put(`${base_url}api/batches/${batchData.id}`, {
                script_status: "completed",
                worker_id: worker_id,
                end_time: formatDateYYYYMMDD(today),
...
async function getData(){
    axios.get(`${base_url}api/batches/pending_everyday`)
    ...
    .catch(function (error) {
        console.log("Error while fetching everyday records");
    })
    .finally(function () {
